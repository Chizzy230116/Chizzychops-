      ...
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .login-bg {
          ...
          background: radial-gradient(ellipse 80% 60% at 50% 0%, rgba(249,115,22,0.12) 0%, transparent 70%), #080B14;
          padding: 1.5rem; font-family: 'DM Sans', system-ui, sans-serif;
        }
        .login-card {
    ...
    <div style={{ minHeight: '100vh', background: '#080B14', color: '#fff', fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <div style={{ background: '#0C1019', borderBottom: '1px solid rgba(255,255,255,0.06)', padding: '0 1.25rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '58px', position: 'sticky', top: 0, zIndex: 50 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
          ...
          {newCnt > 0 && (
            <span className="pulse-dot badge-pill" style={{ background: 'rgba(220,38,38,0.15)', color: '#F87171', border: '1px solid rgba(220,38,38,0.3)', marginLeft: '0.375rem' }}>
              {newCnt} NEW
            </span>
          )}
        </div>
...
function OrdersPanel({ orders, loading, onStatusChange, onDelete, onNotify, onRefresh, todayCount, activeCount, revenue }: {
  ...
  return (
    <div style={{ animation: 'fadein 0.3s ease' }}>
      ...
      {loading ? <SpinLoader text="Loading orders…" /> : shown.length === 0 ? (
                ...
                {isExp && (
                  <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', padding: '1rem 1.125rem' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
                      {cfg.next && (
                        <button className="btn-primary" onClick={() => onStatusChange(order.id, cfg.next!)}>
                          → {STATUS[cfg.next].label}
                        </button>
                      )}
                      {!['delivered', 'cancelled'].includes(order.status) && (
                        <button className="btn-ghost" onClick={() => onStatusChange(order.id, 'cancelled')} style={{ color: '#F87171', borderColor: 'rgba(239,68,68,0.2)', background: 'rgba(239,68,68,0.06)' }}>
                          Cancel
                        </button>
                      )}
...
function MenuPanel({ items, loading, onRefresh, showToast }: { items: MenuItem[]; loading: boolean; onRefresh: () => void; showToast: (m: string, t?: 'ok' | 'err') => void }) {
  ...
  return (
    <div style={{ animation: 'fadein 0.3s ease' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: '0.75rem', marginBottom: '1.5rem' }}>
        {CATS.map(c => (
          <div key={c} className="stat-card" style={{ cursor: 'pointer', transition: 'border-color 0.15s', borderColor: catF === c ? 'rgba(249,115,22,0.4)' : undefined }} onClick={() => setCatF(c)}>
      ...
      <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '1rem', alignItems: 'center' }}>
        ...
        <button className="btn-primary" onClick={() => setEditing({ ...EMPTY, id: 'new-' + Date.now().toString(36), sort_order: items.length + 1 })} style={{ marginLeft: 'auto' }}>
          + Add Item
        </button>
      </div>
          ...
          <table style={{ width: '100%', minWidth: '700px' }}>
            ...
            <tbody>
              {filtered.map((item, idx) => (
                <tr key={item.id} style={{ borderBottom: idx < filtered.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none', transition: 'background 0.1s' }}
                  onMouseEnter={e => (e.currentTarget.style.background = 'rgba(255,255,255,0.02)')}
                  onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                  <td style={{ padding: '0.75rem 1.125rem' }}>
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={item.img_url} alt="" style={{ width: '48px', height: '48px', objectFit: 'cover', borderRadius: '0.625rem', border: '1px solid rgba(255,255,255,0.08)', cursor: 'pointer', display: 'block' }}
                      onClick={() => setPreview({ src: item.img_url, name: item.name })}
                      onError={e => { (e.target as HTMLImageElement).src = 'https://placehold.co/48x48/0F1420/F97316?text=?' }} />
                  </td>
                  <td style={{ padding: '0.75rem 1.125rem' }}>
                  ...
                  <td style={{ padding: '0.75rem 1.125rem' }}>
                    <div style={{ display: 'flex', gap: '0.375rem' }}>
                      <button className="btn-ghost" onClick={() => setPreview({ src: item.img_url, name: item.name })} style={{ padding: '0.3125rem 0.5rem', fontSize: '0.8125rem' }}>👁</button>
...
function InboxPanel({ items, loading, onRefresh }: { items: InboxItem[]; loading: boolean; onRefresh: () => void }) {
  ...
  return (
    <div style={{ animation: 'fadein 0.3s ease' }}>
      ...
      {loading ? <SpinLoader text="Loading inbox…" /> : shown.length === 0 ? (
          ...
          {shown.map((item: any) => {
            ...
            return (
              <div key={item.id} className="card-row" style={{ padding: '1rem 1.125rem' }}>
                ...
                {item._type === 'contact' && (
                  <p style={{ color: 'rgba(255,255,255,0.65)', fontSize: '0.875rem', lineHeight: 1.75, background: 'rgba(255,255,255,0.02)', borderRadius: '0.625rem', padding: '0.75rem' }}>{item.message}</p>
                )}
                {item._type === 'review' && (
...
function EditModal({ item, onClose, onSaved }: { item: Omit<MenuItem, 'updated_at'>; onClose: () => void; onSaved: () => void }) {
  ...
  const save = async () => {
    ...
    if (!form.img_url?.trim()) return setErr('Main image URL is required')
    setSave(true); setErr('')
    try {
    ...
    <>
      ...
      <div style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', width: 'min(720px, calc(100vw - 1.5rem))', maxHeight: '92vh', overflowY: 'auto', background: '#0F1420', border: '1px solid rgba(255,255,255,0.09)', borderRadius: '1.5rem', zIndex: 301, padding: 'clamp(1.25rem,4vw,2rem)', boxShadow: '0 32px 80px rgba(0,0,0,0.7)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
          <h2 style={{ color: '#fff', fontSize: 'clamp(1rem,3vw,1.25rem)', fontWeight: 800, letterSpacing: '-0.02em' }}>{item.name ? `Edit: ${item.name}` : 'New Menu Item'}</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1rem' }}>
          <MF label="Item ID *"><input value={form.id} onChange={e => set('id', e.target.value)} className="input-field" placeholder="s15" /></MF>
          ...
          <MF label="Main Image *" span>
            <div style={{ display: 'flex', gap: '0.875rem', alignItems: 'flex-start' }}>
              <div style={{ flex: 1 }}>
                <input value={form.img_url || ''} onChange={e => set('img_url', e.target.value)} className="input-field" style={{ marginBottom: '0.5rem' }} placeholder="/images/egusi.jpg or https://…" />
