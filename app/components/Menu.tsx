  ...
  return (
    <section id="menu" className="section" style={{ background: 'var(--brand-surface)', position: 'relative', overflow: 'hidden' }}>
      ...
      <div className="container" style={{ position: 'relative', zIndex: 1 }}>
        ...
        {/* ── Delivery Notice — Redesigned Bento Card ───── */}
        <div style={{
          display: 'grid',
          ...
          background: 'linear-gradient(135deg, rgba(249,115,22,0.08) 0%, rgba(220,38,38,0.04) 100%)',
          ...
          <div>
            ...
            <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: '0.875rem', lineHeight: 1.75, margin: 0 }}>
              Every order cooked from scratch — no shortcuts, no reheated meals. Just real, soulful Nigerian food
              made with care. Please allow <strong style={{ color: '#FBBF24', fontWeight: 800 }}>up to 6 hours</strong> for delivery.
            </p>
          </div>
          ...
          <style>{`
            @media (max-width: 600px) {
              #delivery-notice { grid-template-columns: auto 1fr !important; }
              #delivery-notice-badges { display: none !important; }
            }
          `}</style>
        </div>

        {/* ── Category Filter — Elevated Pill Group ───────  */}
        <div style={{
          ...
          {CATS.map(c => (
            <button
              ...
              style={{
                ...
                transition: 'all 0.2s',
                background: cat === c ? 'linear-gradient(135deg, #F97316, #DC2626)' : 'transparent',
                color: cat === c ? '#fff' : 'rgba(255,255,255,0.45)',
                boxShadow: cat === c ? '0 4px 16px rgba(249,115,22,0.3)' : 'none',
                transform: cat === c ? 'scale(1)' : 'scale(0.97)',
              }}
            >
              {c}
            </button>
            ...
            <div style={{
              ...
              {displayed.map(item => (
                <MenuCard
                  key={item.id}
                  item={item}
                  inCart={cart.find(c => c.id === item.id)?.qty || 0}
        ...
        <div style={{
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          gap: '0.5rem', paddingTop: '0.5rem',
        }}>
          <div style={{ height: '1px', flex: 1, background: 'rgba(255,255,255,0.06)' }} />
          <p style={{ color: 'rgba(249,115,22,0.5)', fontSize: '0.8125rem', fontStyle: 'italic', whiteSpace: 'nowrap' }}>
            💬 All items can be customised — WhatsApp us!
          </p>
          <div style={{ height: '1px', flex: 1, background: 'rgba(255,255,255,0.06)' }} />
        </div>
      </div>
...
function MenuCard({ item, inCart, onAdd }: { item: Item; inCart: number; onAdd: () => void }) {
  ...
  return (
    <div style={{
      ...
      overflow: 'hidden',
      background: 'var(--brand-card)',
      border: `1px solid ${inCart > 0 ? 'rgba(249,115,22,0.5)' : 'rgba(255,255,255,0.06)'}`,
      display: 'flex', flexDirection: 'column',
      transition: 'all 0.3s ease',
      boxShadow: inCart > 0
        ? '0 0 0 1px rgba(249,115,22,0.15), 0 8px 32px rgba(249,115,22,0.12)'
    ...
    onMouseLeave={e => {
      e.currentTarget.style.border = `1px solid ${inCart > 0 ? 'rgba(249,115,22,0.5)' : 'rgba(255,255,255,0.06)'}`
      e.currentTarget.style.boxShadow = inCart > 0
        ? '0 0 0 1px rgba(249,115,22,0.15), 0 8px 32px rgba(249,115,22,0.12)'
        : '0 2px 16px rgba(0,0,0,0.25)'
      e.currentTarget.style.transform = 'translateY(0)'
    }}
    >
      {/* ── Image Area ──────────────────────────────────── */}
      <div
        style={{ position: 'relative', height: '190px', overflow: 'hidden', flexShrink: 0, cursor: hasTwo ? 'pointer' : 'default' }}
      ...
      >
        ...
        <img src={item.img} alt={item.name} style={{
          ...
          transform: showSecond ? 'scale(1.06)' : 'scale(1)',
        }}
          onError={e => { (e.target as HTMLImageElement).src = 'https://placehold.co/280x190/1A0800/F97316?text=No+Image' }}
        />
        {hasTwo && (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={item.img2} alt={`${item.name} 2`} style={{
            position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover',
            transition: 'opacity 0.65s ease, transform 0.55s ease',
            opacity: showSecond ? 1 : 0,
            transform: showSecond ? 'scale(1)' : 'scale(1.06)',
          }}
          ...
          <div style={{
            ...
            {[0, 1].map(i => (
              <span key={i} style={{
                ...
                background: (i === 0 && !showSecond) || (i === 1 && showSecond) ? '#fff' : 'rgba(255,255,255,0.35)',
        ...
        <div style={{
          ...
          background: 'linear-gradient(to top, rgba(10,3,0,0.92) 0%, rgba(10,3,0,0.4) 45%, transparent 75%)',
        }} />
        ...
        {item.badge && (
          <span style={{
            ...
            boxShadow: `0 2px 10px ${badgeColor}55`,
          }}>
            {item.badge}
          </span>
        )}
        ...
        {/* Price — bottom-left, large and clear */}
        <div style={{ position: 'absolute', bottom: '12px', left: '14px', zIndex: 2 }}>
          <span style={{
            color: '#FBBF24', fontWeight: 900, fontSize: '1.125rem',
      ...
      <div style={{ padding: '1.125rem 1.125rem 1rem', display: 'flex', flexDirection: 'column', gap: '0.375rem', flex: 1 }}>
        ...
        <h3 style={{
          color: '#fff', fontFamily: 'var(--font-playfair)',
          fontWeight: 700, fontSize: '1rem', lineHeight: 1.3, margin: 0,
        }}>
          {item.name}
        </h3>
        ...
        <button
          ...
          style={{
            ...
            border: `1px solid ${inCart > 0 ? 'rgba(249,115,22,0.45)' : 'rgba(255,255,255,0.1)'}`,
            cursor: 'pointer', fontWeight: 800, fontSize: '0.875rem',
            transition: 'all 0.2s',
            background: inCart > 0
              ? 'linear-gradient(135deg, rgba(249,115,22,0.18), rgba(220,38,38,0.12))'
              : 'rgba(255,255,255,0.04)',
            color: inCart > 0 ? '#F97316' : 'rgba(255,255,255,0.6)',
            letterSpacing: '0.01em',
          }}
          onMouseEnter={e => {
            e.currentTarget.style.background = 'linear-gradient(135deg, rgba(249,115,22,0.25), rgba(220,38,38,0.18))'
            e.currentTarget.style.color = '#F97316'
            e.currentTarget.style.border = '1px solid rgba(249,115,22,0.5)'
            e.currentTarget.style.transform = 'scale(1.01)'
          }}
          onMouseLeave={e => {
            e.currentTarget.style.background = inCart > 0
              ? 'linear-gradient(135deg, rgba(249,115,22,0.18), rgba(220,38,38,0.12))'
              : 'rgba(255,255,255,0.04)'
            e.currentTarget.style.color = inCart > 0 ? '#F97316' : 'rgba(255,255,255,0.6)'
            e.currentTarget.style.border = `1px solid ${inCart > 0 ? 'rgba(249,115,22,0.45)' : 'rgba(255,255,255,0.1)'}`
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div>
            <h3 style={{
              color: '#fff', fontFamily: 'var(--font-playfair)',
              fontWeight: 700, fontSize: '1.25rem', margin: '0 0 0.2rem',
            }}>
              Your Order
            </h3>
            <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '0.8125rem', margin: 0 }}>
              {cart.reduce((s, c) => s + c.qty, 0)} item{cart.reduce((s, c) => s + c.qty, 0) !== 1 ? 's' : ''} selected
            </p>
          </div>
        ...
        <div style={{
          ...
          <p style={{ color: 'rgba(255,255,255,0.55)', fontSize: '0.75rem', lineHeight: 1.7, margin: 0 }}>
            ...
            Please allow up to <strong style={{ color: '#FBBF24', fontWeight: 800 }}>6 hours</strong> for delivery.
          </p>
        </div>
            ...
            <div key={item.id} style={{
              ...
              <div style={{
                ...
                <img
                  ...
                  onError={e => { (e.target as HTMLImageElement).src = 'https://placehold.co/56x56/1A0800/F97316?text=?' }}
                />
              </div>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <p style={{ color: '#FBBF24', fontWeight: 800, fontSize: '0.875rem', margin: 0 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexShrink: 0 }}>
                <button
                  ...
                  style={{
                    ...
                    color: item.qty === 1 ? 'rgba(220,38,38,0.8)' : 'rgba(255,255,255,0.7)',
                    cursor: 'pointer', fontSize: '1rem',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    transition: 'all 0.15s',
                  }}
                  onMouseEnter={e => {
                    e.currentTarget.style.background = item.qty === 1 ? 'rgba(220,38,38,0.12)' : 'rgba(255,255,255,0.1)'
                    e.currentTarget.style.borderColor = item.qty === 1 ? 'rgba(220,38,38,0.35)' : 'rgba(255,255,255,0.2)'
                  }}
                  onMouseLeave={e => {
                ...
                <button
                  ...
                  style={{
                    ...
                    color: '#F97316', cursor: 'pointer', fontSize: '1.125rem',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    transition: 'all 0.15s',
                  }}
                  onMouseEnter={e => {
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div>
              <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.75rem', fontWeight: 600, margin: '0 0 0.125rem', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                Order Total
              </p>
            ...
            <span style={{
              color: '#FBBF24', fontWeight: 900, fontSize: '1.5rem',
              fontFamily: 'var(--font-playfair)', letterSpacing: '-0.02em',
            }}>
              {formatPrice(total)}
            </span>
          </div>
          ...
          <button
            ...
            onMouseEnter={e => {
              e.currentTarget.style.background = '#20c05c'
              e.currentTarget.style.boxShadow = '0 8px 36px rgba(37,211,102,0.42)'
              e.currentTarget.style.transform = 'translateY(-1px)'
            }}
            onMouseLeave={e => {
              e.currentTarget.style.background = '#25D366'
              e.currentTarget.style.boxShadow = '0 6px 28px rgba(37,211,102,0.3)'
              e.currentTarget.style.transform = 'translateY(0)'
            }}
          >
          ...
          <p style={{
            ...
            fontSize: '0.75rem', marginTop: '0.75rem', lineHeight: 1.5,
          }}>
